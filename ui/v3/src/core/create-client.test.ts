...
it.each([
  ...
  "retains a persisted $format-ID Home Screen through cache updates",
  async ({ id }) => {
...
it.each(["/updated.jpg", null])(
  "keeps scene media paths complete when a feed summary updates screenshot to %s",
  (screenshot) => {
    ...
    expect(
      cache.readQuery({ query: scenePathsQuery })?.findScene.paths,
    ).toEqual({
      ...paths,
      screenshot,
    });
