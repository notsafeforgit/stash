...
- `-hls_segment_options movflags=+frag_discont` — preserves those absolute timestamps inside each fMP4 fragment. Without this MP4 muxer option, `tfdt` starts at zero for every new ffmpeg run despite `output_ts_offset`; seeking mixes incompatible cached timelines and leaves audio gaps. Keep the default edit lists enabled. This does not add `EXT-X-DISCONTINUITY` to the playlist. `stream_v3_timestamps_test.go` checks real encoded packets across restarts using the original init segments.
...
**iOS startup regression (2026-09-21)** — The frame-count keyframe / disabled-negative-timestamp change in `4f1180550`, together with the fallback-rate follow-up in `1abd82f7f`, was reverted after iOS HLS transcodes were reported to freeze around one second. The initial AAC fragment contained an unsigned `tfdt` of `2^64-1024` after negative encoder priming wrapped around. ffprobe interpreted the value as a signed packet timestamp, so packet-continuity tests missed it; Linux Chromium/WebKit playback checks also passed. The timestamp regression now reads the raw MP4 box as unsigned before probing packets. Keep the previous HLS timing behavior until a replacement is checked on actual iOS playback; the original repeated-seek frame-pacing issue remains unresolved.
...
**Audio continuity and lightbox lifetime**: `scene-carousel.tsx` replaces YARL's per-slide-keyed carousel with three stable slots through its public module API. The center retains one `ScenePlayer`, store and native video across scene/marker swipes; neighboring slots render posters. YARL still owns dragging and animation. `playbackKey` resets scene/marker state without resetting media ownership. Pending queries/OPFS and loading sentinels suspend the retained player and release its source; late query results cannot replace the selected scene. Audio, playback rate and an active held-speed gesture survive navigation. Keep WebKit's explicit `canplay` resume. `usePlayDelay` retries muted only after a current `NotAllowedError`; new selections cancel old animation deadlines and seeks. Chromium/WebKit tests exercise the actual lightbox, but physical iPhone autoplay permission and MMS behavior still require device verification.
