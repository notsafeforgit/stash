...
const sortByOptions = [
  ...
  .concat([
    // Backend sort field is `favorite` (matches the GraphQL enum
    // `PerformerSortFieldEnum.FAVORITE`); locale key is `favourite`
    // (en-GB convention shared with v2.5 — every translation file
    // ships the spelling under that key, not `favorite`). Without
    // this messageID override the lookup falls back to the value
    // "favorite" lowercase.
    { messageID: "favourite", value: "favorite" },
    {
