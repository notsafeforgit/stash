...
export const PerformerAliasListInput: React.FC<IPerformerAliasListInputProps> = (props) => {
  const values = props.value.concat({ alias: "", ignore_auto_tag: true });
  const [draggedIdx, setDraggedIdx] = useState<number | null>(null);

  function valueChanged(idx: number, field: "alias" | "ignore_auto_tag", value: any) {
      ...
      <div className={`string-list-input ${props.errors ? "is-invalid" : ""}`}>
        <Form.Group>
          {values.map((v, i) => (
            ...
            >
              <Form.Control
                className={`text-input ${props.errorIdx?.includes(i) ? "is-invalid" : ""}`}
                value={v.alias}
              ...
              <div className="d-flex align-items-center ml-3 mr-3">
                <Form.Check
                  ...
                  custom
                  type="switch"
                  label={<FormattedMessage id="auto_tag" defaultMessage="Auto tag" />}
