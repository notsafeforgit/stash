...
export type Option = string | number | OptionWithLabel;

export function resolveOption(o: Option): OptionWithLabel {
  if (typeof o === "object") return o;
  const s = o.toString();
  return { value: s, messageID: s };
}
...
export interface SavedCriterion<T> {
...
> extends Criterion {
  ...
  /** Label for use in a dropdown option (capitalised, not sentence-fragment form). */
  public static getModifierOptionLabel(
    intl: IntlShape,
    modifier: CriterionModifier,
  ) {
    const optionMessageIDs: Partial<Record<CriterionModifier, string>> = {
      [CriterionModifier.Equals]: "criterion_modifier_values.exactly",
      [CriterionModifier.NotEquals]: "criterion_modifier_values.not_equals",
      [CriterionModifier.GreaterThan]: "criterion_modifier_values.greater_than",
      [CriterionModifier.LessThan]: "criterion_modifier_values.less_than",
      [CriterionModifier.IsNull]: "criterion_modifier_values.none",
      [CriterionModifier.NotNull]: "criterion_modifier_values.any",
      [CriterionModifier.Includes]: "criterion_modifier_values.includes",
      [CriterionModifier.IncludesAll]: "criterion_modifier_values.includes_all",
      [CriterionModifier.Excludes]: "criterion_modifier_values.excludes",
      [CriterionModifier.MatchesRegex]:
        "criterion_modifier_values.matches_regex",
      [CriterionModifier.NotMatchesRegex]:
        "criterion_modifier_values.not_matches_regex",
      [CriterionModifier.Between]: "criterion_modifier_values.between",
      [CriterionModifier.NotBetween]: "criterion_modifier_values.not_between",
    };
    const id = optionMessageIDs[modifier];
    return id
...
interface CriterionOptionParams {
  messageID: string;
  type: CriterionType;
  makeCriterion: MakeCriterionFn;
  hidden?: boolean;
  sfwMessageID?: string;
  // Override the GraphQL filter field name used when serializing this criterion
  // into an AST condition. Defaults to `type`. Used when multiple client
  // criterion types map to the same server-side filter slot — e.g. the single
  // and multi forms of custom_fields both serialize to `custom_fields`.
  serverField?: string;
}
...
interface ModifierCriterionOptionParams extends CriterionOptionParams {
  inputType?: InputType;
...
export class ModifierCriterionOption extends CriterionOption {
  ...
  public readonly sortOptions: boolean;

  constructor(options: ModifierCriterionOptionParams) {
    super(options);
    ...
    this.sortOptions = options.sortOptions ?? false;
  }
}
...
export class IHierarchicalLabeledIdCriterion extends ModifierCriterion<IHierarchicalLabelValue> {
  ...
  public setFromSavedCriterion(
    criterion: SavedCriterion<IHierarchicalLabelValue>,
  ) {
...
export class StringCriterionOption extends ModifierCriterionOption {
  constructor(
    options: Partial<
      Omit<ModifierCriterionOptionParams, "messageID" | "type">
    > &
      Pick<ModifierCriterionOptionParams, "messageID" | "type">,
  ) {
    super({
      modifierOptions: [
...
export class StringCriterion extends ModifierCriterion<string> {
  constructor(type: ModifierCriterionOption) {
    super(type, type.options?.[0]?.toString() ?? "");
  }
...
export class DurationCriterion extends ModifierCriterion<INumberValue> {
  ...
  protected getLabelValue(_intl: IntlShape) {
    const value = secondsToTimestamp(this.value.value ?? 0);
    const value2 = secondsToTimestamp(this.value.value2 ?? 0);
    if (
      this.modifier === CriterionModifier.Between ||
      this.modifier === CriterionModifier.NotBetween
    ) {
