...
const detail = {
  ...
  paths: { screenshot: "/old.jpg", stream: "/stream", sprite: "/sprite.jpg" },
  preview_image: null,
};
...
it.each([
  ...
  JobStatus.Cancelled,
  null,
])("publishes only artwork after %s, even after the initiating view leaves", async (status) => {
  vi.useFakeTimers();
  let finished = false;
