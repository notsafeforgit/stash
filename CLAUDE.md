...
**Audio continuity and lightbox lifetime**: the native element can retain its audio state across direct/HLS, quality, source-reload and automatic source changes. WebKit still needs the existing explicit `canplay` resume; do not replace that choreography with reliance on the `autoplay` attribute alone. `usePlayDelay` retries muted only after `NotAllowedError`, and ignores a rejection after unmount, a source change or an explicit user playback gesture. `AbortError` during a source change must never mute the next video. The lightbox still mounts a player per active scene/marker slide, so retaining the same element across actual swipes requires a separate carousel/session ownership change. Automated Chromium/WebKit audio fixtures verify the media adapter; they do not establish physical iPhone autoplay permission or MMS behavior.
