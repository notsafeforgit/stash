...
- `-force_key_frames expr:gte(n,n_forced*gopFrames)` — count output frames so fractional frame rates do not round keyframes a frame late. Together with `-g` and `-keyint_min`, this keeps segment frame counts consistent across runs.
...
- `-avoid_negative_ts disabled` for full transcodes — the MP4 edit list handles encoder delay. `make_non_negative` shifted only the initial run, putting its frames later than restarted runs and creating overlaps when cached segments met. Codec-copy paths retain `make_non_negative`; `make_zero` would erase their absolute timeline.
